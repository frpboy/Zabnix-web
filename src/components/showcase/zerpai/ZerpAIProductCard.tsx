...
import { ZerpAIInteractiveProductDemo } from "./ZerpAIInteractiveProductDemo";

const FALLBACK_CAPABILITY_ICONS: LucideIcon[] = [
  Package,
  ShoppingCart,
  Landmark,
  Sparkles,
...
function getCapabilityIcon(title: string, fallbackIndex: number) {
  ...
  if (normalized.includes("ai") || normalized.includes("forecast")) {
    return Sparkles;
  }

  return FALLBACK_CAPABILITY_ICONS[fallbackIndex] ?? Package;
}

type ZerpAIProductCardProps = {
  product: Product;
};

export function ZerpAIProductCard({ product }: ZerpAIProductCardProps) {
  ...
  const mobileCapabilities = useMemo<MobileProductCapability[]>(
    () =>
      product.features.slice(0, 4).map((feature, index) => ({
        ...
        icon: getCapabilityIcon(feature.title, index),
      })),
    [product.features]
  );
    ...
    <>
        ...
        <div className="relative grid items-center gap-9 lg:grid-cols-[0.78fr_1.22fr] lg:gap-12">
          <div className="min-w-0">
            ...
            <ul className="mt-7 space-y-4">
              {mobileCapabilities.map(({ title, icon: Icon }) => (
                <li key={title} className="flex items-center gap-3 text-sm text-body">
                  <Icon
                    size={21}
                    strokeWidth={2}
      ...
      <div className="space-y-0 lg:hidden">
        <MobileProductPresentation
          ...
          viewHref="/products/zerpai/demo"
          onRequestDemo={() => setOpen(true)}
        />
        <ZerpAIInteractiveProductDemo
          product={product}
          onRequestDemo={() => setOpen(true)}
        />
      </div>
